...
GENERIC_VECTOR generic_vector_init_default(ITEM (*init_copy)(ITEM),
                                           void (*destroy)(ITEM *)) {
...
Status generic_vector_push_back(GENERIC_VECTOR hVector, ITEM hItem) {
  ...
  if (pVector->size >=
      pVector->capacity) // if there is not enough room then try to make room
  {
...
GENERIC_VECTOR generic_vector_init_copy(GENERIC_VECTOR hVector) {
  int i;
  Generic_vector *pVector = (Generic_vector *)hVector;
  Generic_vector *pCopy = generic_vector_init_default(pVector->init_copy,pVector->destroy);
  for(i = 0; i < generic_vector_get_size(hVector); i++){
    generic_vector_push_back(pCopy, *generic_vector_at(hVector, i));
  }
  return pCopy;
}
